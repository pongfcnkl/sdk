...
def PlayDataCallBackWin(lPlayHandle, dwDataType, pBuffer, dwBufSize, pUser):
    ...
    if dwDataType == NET_DVR_SYSHEAD:
        ...
        if Playctrldll.PlayM4_InputData(PlayCtrl_Port, pBuffer, dwBufSize):
            print('数据输入成功')
        else:
...
def OpenPlayback(lUserId, lChannel, command):
    ...
    net_dvr_vod_para.struBeginTime.dwDay = 23
    ...
    net_dvr_vod_para.struEndTime.dwDay = 23
    net_dvr_vod_para.struEndTime.dwHour = 0
    net_dvr_vod_para.struEndTime.dwMinute = 1
...
def CleanPlayBackUp(Objdll, Playctrldll, iPlayBack, lUserId):
